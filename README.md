...
文本消息下方会挂上按钮：作品信息挂 `再来一张`，帮助文案挂 `随机一张 / 随机原图 / 随机小图`。由 `show_buttons` 控制，默认开。

### 为什么只能挂在文本上

**图片消息挂不了按钮**，这是平台限制而非取舍：官方说明按钮是"在 markdown 消息的基础上"挂载，而运行时对带 media 的消息走 `msg_type: 7` 分支，**直接丢弃 keyboard 字段**（且不报错）。所以按钮一律挂在文本消息上，该消息会被自动升级为 markdown。

### 用的是指令按钮，不是回调按钮

按键的 `action.type` 有三种，这里用的是 `2`（指令按钮），理由是它明显更省事也更稳：

| | `type=2` 指令按钮（本项目采用） | `type=1` 回调按钮 |
| --- | --- | --- |
| 点击后 | 客户端插入 `@bot <指令>` 并直接发送 | 推送 `INTERACTION_CREATE` 给后端 |
| 群聊能否命中 | 能——插入的 `@bot` 让它成为 at_message | 能 |
| 需要写 `buttons` 处理器 | **不需要** | 需要 |
| 时限 | 无 | 官方要求 **3 秒内**回应，否则用户端一直 loading |
| 新增代码 | 零，复用现有命令路径 | 一个处理器 + ack 时序处理 |

回调按钮那条路要在这条链路上查 API 再发图，网络往返很容易贴近 3 秒；而运行时的 ack 发生在**处理器跑完之后**，要规避就得在处理器开头手动 `interaction.ack(0)`。指令按钮没有这个问题，还顺带省掉了"群聊必须 @"这一层——插入的 `@bot` 已经满足了。

### 发送顺序

`message_order` 决定文本与图片的先后，按钮始终紧贴文本下方：

- `text_first`（默认）：`[作品信息 + 按钮]` → `[图片]`，按钮夹在中间；
- `image_first`：`[图片]` → `[作品信息 + 按钮]`，按钮落在最底部，按起来少滚一点。

`image_first` 下若图片发送失败，**不会发文案**——描述一张没发出去的图没有意义，此时错误会直接透给用户；`text_first` 下文案已发出，则只记日志不刷屏。

### 权限与兜底

内嵌按钮需要 bot 侧开通（官方文档把自定义按钮标为「内邀开通」，按钮模版则是「申请使用」，两处口径不完全一致）。未开通时平台可能整条拒收，所以**带按钮发送失败会自动去掉按钮重试一次**，最坏退化成没有按钮的纯文本，不会把文案一起赔进去。日志里会留下 `带按钮的文本发送失败，去掉按钮重试`。

若反复出现该 warn，把 `show_buttons` 关掉即可。
...
| 文本下方没有按钮 | 先确认 `show_buttons` 没被关；再 tail 搜 `带按钮的文本发送失败`——有这条说明 bot 没有按钮权限，插件已自动退回纯文本 |
| 按钮点了没反应 | 指令按钮要客户端版本支持（官方提到 `enter` 需 8983+）。注意 SDK 的类型注释把 `enter` 标为"仅单聊"，官方接口表则未限定场景——**群聊下的行为两处口径不一致，未实测**。若群聊点按钮只填充输入框不发送，属预期内的兼容差异，手动按发送即可 |
