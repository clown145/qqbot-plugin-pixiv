...
| `max_image_mb` | number | `2` | 单张图片体积上限（MB），超过则回落小一档；范围 0.5–6 |
...
阈值即面板配置 `max_image_mb`，默认 **2 MB**（约 5.0 ms 序列化，合计约 7.5 ms，留 25% 余量）。偏向保守是因为两种错法的代价不对称：**阈值偏小只是偶尔回落小一档（优雅降级），偏大则直接撞 `Error 1102` 让整条消息失败**。上线后用 `wrangler tail` 确认无 `exceededCpu`，再把 `max_image_mb` 逐步调高（面板上限 6 MB；这个上限只是防止把配置填成荒唐值，并不代表 6 MB 在 Free 上能跑通——按上面的推导，超过约 3 MB 就必然超限）。

**升级到 Paid 套餐后该限制完全消失**（默认 30 s CPU），可直接把 `max_image_mb` 拉满或删掉回落逻辑。若想彻底绕开体积与 CPU 的耦合，用上面的 URL 直传更省事。
